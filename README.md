Prospector-api Business Data Enrichment
...
One for Firebase Admin SDK
One for Google Sheets/Drive access
...
Batching Model
Each job processes results in batches (default 30 items per batch)
Client or Vercel cron can trigger the next batch by POSTing to /api/jobs/{id}
This model works well with Vercel's serverless function time limits
Testing
To test locally:
...
The batch processing model is designed to work within Vercel's serverless time limits (max 60 seconds per function)
For large jobs, the client needs to periodically trigger the next batch
